...
| 선 클릭 | 그 선행관계 끊기 |
| 노드 클릭 | 오른쪽 상세 패널에서 부서·담당자·상태·산출물·막대 색 수정 |
| `이슈 메모` 버튼 | 화면 가운데에 쟁점 스티커를 붙임 |
| 스티커 머리띠 끌기 | 그 이슈의 시점·위치 이동 |
| 스티커의 메모장 버튼 | 30px 아이콘으로 줄임. 아이콘을 누르면 다시 펼쳐짐 |
...
메모가 일정 막대를 가리면 머리띠의 메모장 버튼으로 **30px 아이콘**까지 줄입니다. 아이콘은 끌어서 옮길 수 있고(3px 넘게 움직이면 이동, 제자리에서 떼면 펼침), 마우스를 올리면 전체 내용이 툴팁으로 보입니다. 줄인 상태는 `notes/<id>.min` 으로 저장돼 다른 참석자 화면에서도 같이 줄어듭니다.

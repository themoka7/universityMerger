...
| 아래 범례의 단계 클릭 | 그 단계 과제 전체의 막대 색 변경 |
...
- **단계별 일괄**: 캔버스 아래 범례에서 단계를 누르면 색 고르개가 열립니다. 24색 견본과 직접 고르기(색상 선택기)가 있고, `기본값`으로 되돌립니다. 이 값은 공유 저장소(`meta/board.phaseColors`)에 남아 모든 참석자에게 같이 적용됩니다.
- **과제 하나만**: 노드를 선택하고 상세 패널의 `막대 색`에서 지정합니다. 지정하면 단계 색보다 우선하고, `기본값`을 누르면 다시 단계 색을 따릅니다.

고른 색의 밝기에 따라 막대 글자를 흰색·검정 중에서 자동으로 골라 대비를 유지합니다.

...
가로축이 시간, 세로로는 **주관부서별 구역**이 나뉩니다. 폼을 채우는 대신 캔버스에서 바로 잡습니다.

| 동작 | 결과 |
| --- | --- |
| 부서 구역의 빈 곳을 좌우로 끌기 | 그 부서의 새 과제 생성. 끈 구간이 기간(주 단위로 맞춰짐) |
| 노드 끌기 | 일정 이동. 다른 부서 구역으로 옮겨도 주관부서는 상세 패널에서 바꿉니다 |
| 노드 좌·우 끝 끌기 | 시작일·종료일 조절 |
| 노드 오른쪽 동그라미 → 다른 노드 | 선행관계 연결 (가장자리에서 자동 스크롤) |
| 선 클릭 | 그 선행관계 끊기 |
| 노드 클릭 | 오른쪽 패널에서 부서·담당자·상태·산출물·막대 색 수정 |
| 메모 클릭 | 오른쪽 패널이 `메모`로 바뀌어 내용·시점·색·접기·삭제 |
| `이슈 메모` 버튼 | 화면 가운데에 쟁점 스티커를 붙임 |
| 메모 끌기 | 그 이슈의 시점·위치 이동 |
| 오른쪽 패널의 `캔버스 표시` | 30px 메모장 아이콘으로 접기 / 다시 펼치기 |
| `Delete` / `Esc` | 선택 과제 삭제 / 선택 해제 |
| 부서별 정렬 | 부서 구역별로 겹치지 않게 재배치 |

배율은 분기·월·주 3단계. 순환 참조(A→B→A)는 만들 수 없습니다.

**이슈 메모** — 일정이 아니라 회의에서 나온 쟁점을 시점 위에 붙여 두는 용도입니다. 내용을 눌러 바로 쓰고, 머리띠의 `◧`로 8색 파스텔 중에 고르고, `×`로 지웁니다. 공유 저장소의 `notes` 컬렉션에 남아 다른 참석자에게도 보입니다.

캔버스의 메모는 **표시 전용**입니다. 누르면 오른쪽 패널이 `메모`로 바뀌고 거기서 내용·시점·색을 고칩니다(과제 노드와 같은 방식). 일정 막대를 가리면 패널의 `캔버스 표시`로 **30px 메모장 아이콘**까지 접습니다. 접힌 상태에서도 끌어서 옮길 수 있고(3px 넘게 움직이면 이동, 제자리에서 떼면 선택), 마우스를 올리면 전체 내용이 툴팁으로 보입니다. 접은 상태는 `notes/<id>.min` 으로 저장돼 다른 참석자 화면에서도 같이 접힙니다.

**Firestore 규칙에 `notes` 컬렉션이 빠져 있으면** 메모가 저장되지 않습니다. 그 경우 화면에는 즉시 반영되지만 새로고침하면 사라지고, 상단에 규칙을 고치라는 안내가 뜹니다.

**색 바꾸기** — 두 갈래입니다.

- **부서별 일괄**: 캔버스 아래 범례에서 부서를 누르면 색 고르개가 열립니다. 24색 견본과 직접 고르기가 있고, `기본값`으로 되돌립니다. 공유 저장소(`meta/board.deptColors`)에 남아 모든 참석자에게 같이 적용됩니다.
- **과제 하나만**: 노드를 선택하고 상세 패널의 `막대 색`에서 지정합니다. 부서 색보다 우선합니다.

기본 색은 부서 이름순으로 12색 팔레트에서 배정합니다. 고른 색의 밝기에 따라 막대 글자를 흰색·검정 중에서 자동으로 골라 대비를 유지합니다.

### 그 밖에

- **과제 목록** — 과제·주관부서·담당자·기간·상태 표, 산출물/협조부서/선행과제 포함
...
```
tasks/<id>   { title, phase, dept, coop, owner, start, end, status, deps[], y, color, deliverable, note, updatedAt }
meta/board   { org, title, target, phaseColors }
```

- `deps` — 선행과제 id 배열
- `y` — 캔버스 세로 위치(px). 없으면 단계별로 자동 배치
- `color` — 이 과제만의 막대 색(`#RRGGBB`). 없으면 `phaseColors` → 기본 단계 색 순으로 적용
- `phaseColors` — 단계 id별 색 지정. 지정하지 않은 단계는 기본 색(다크 모드 대응)을 씁니다
